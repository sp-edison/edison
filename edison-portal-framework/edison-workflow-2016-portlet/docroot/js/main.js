  ...
  if(typeof isNotLoaded !== 'undefined' && isNotLoaded){
    ...
    $(document).ready(function () {
          ...
          $.each(ports, function(_, port){
            ...
            var endPoint = jsPlumbInstance.addEndpoint(
                ...
                {anchor: defaultAnchor, uuid: endPointGuid, 
                  overlays: [["Label", {label: port["name"], location:labelLocation}]]}, 
                endPointType);
      ...
      function drawWorkflowInstanceStatus(workflowStatus){
        ...
        var currentCanvasId = "#"+ $(currentJsPlumbInstance.getContainer()).attr("id");
        ...
        for(var i=0; i<simulations.length; i++){
          ...
          $(clientId).removeClass("waitingbox");
      ...
      function drawWorkflowInstanceJstree(instanceJstree){
        console.log(instanceJstree);
        ...
        $("#workflow-instance-list").jstree({
          ...
          $('#workflow-instance-list').jstree("open_all");
        }).bind("select_node.jstree", function (event, data) {
          var nodeId = data.node.id;
          var node = data.node;
          if(node.type !== "instance"){
            if(!$("#" + nodeId).hasClass("jstree-open")){
              ...
              $("#workflow-instance-list").jstree("close_node", node);
            }
          }
        });
      }
      
      function updateWorkflowInstanceJstree(instanceJstree){
        $('#workflow-instance-list').jstree(true).settings.core.data = instanceJstree;
        $('#workflow-instance-list').jstree(true).refresh();
      }
      ...
      function updateWorkflowInstanceStatus(workflowInstanceId, interval){
        ...
        workflowStatusTimer = setTimeout(function(){
          synchronousAjaxHelper.get(
              "/delegate/services/workflows/" + workflowInstanceId +"/siblings",
              function(instanceJstree){
                if(currentWorkflowInstanceId && currentWorkflowInstanceId == workflowInstanceId){
                  updateWorkflowInstanceJstree(instanceJstree);
                }else{
                  drawWorkflowInstanceJstree(instanceJstree);
                }
              });
          currentWorkflowInstanceId = workflowInstanceId;
          synchronousAjaxHelper.get(
              "/delegate/services/workflows/" + workflowInstanceId +"/status",
        ...
        if($("#workflow-instance-name-input").val()){
          ...
          var workflowRquestJson = synchronousAjaxHelper.post(
              ...
              function(workflowInstance){
                ...
                if(workflowInstance){
                  loadWorkflowDefinition(localWorkflow["workflowId"], currentJsPlumbInstance, workflowInstance["workflowInstanceId"]);
                }
              });
