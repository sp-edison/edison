...
/*  @RequestMapping(value ="/editor/{portletId}/plid/{plid}")
  public @ResponseBody String getScienceAppPortEditorsPortletUrl(
      @PathVariable("portletId") String portletId,
      @PathVariable("plid") long plid,
      HttpServletRequest request)
          throws SystemException, PortalException{
    try{
      log.info("portletId" + portletId);
      log.info("plid" + plid);
      return "{url:'"+ PortletURLFactoryUtil
          .create(request, portletId, plid, PortletRequest.RENDER_PHASE)
          .toString() + "'}";
    }catch (Exception e){
      log.error("error", e);
      throw e;
