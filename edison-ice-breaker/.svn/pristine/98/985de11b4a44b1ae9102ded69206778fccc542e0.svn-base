...
public class BootstrapDataPopulator implements ApplicationContextAware, InitializingBean {
	...
	private void createDefaultUsers(List<Cluster> clusters, boolean roleCreation) throws Exception {
		...
		User sonic0913 = new User("sonic0913", new Sha256Hash("gmlwjd").toHex(), "sonic0913@naver.com", "", "", "");
		sonic0913.setSessionToken("");
		sonic0913.setStorageSource(Cloud.getInstance().getProp("data.basedir") + "/" + sonic0913.getUserId());
		sonic0913.setUserName("유정록");
		sonic0913.setAffiliation("KISTI");
		sonic0913.setMajor("KISTI");
		sonic0913.setClassName("KISTI");
		sonic0913.setUid("4050078");
		sonic0913.setGid("4050078");
		sonic0913.setRegisteredDate(new Date());
		Set<Role> roles_user = new HashSet<Role>();
		roles_user.add(roleDAO.findRole(session, Cloud.getInstance().getProp("user.role.normal")));
		sonic0913.setRoles(roles_user);
		sonic0913.setState(UserState.ACTIVATED);
		session.save(sonic0913);
		AuthUtils.createDir(clusters, sonic0913.getUserId());
		
		User test = new User("test", new Sha256Hash("test").toHex(), "test@naver.com", "", "", "");
		test.setSessionToken("");
		test.setStorageSource(Cloud.getInstance().getProp("data.basedir") + "/" + test.getUserId());
		test.setUserName("유정록");
		test.setAffiliation("KISTI");
		test.setMajor("KISTI");
		test.setClassName("KISTI");
		test.setUid("4050078");
		test.setGid("4050078");
		test.setRegisteredDate(new Date());
		Set<Role> roles_user1 = new HashSet<Role>();
		roles_user1.add(roleDAO.findRole(session, Cloud.getInstance().getProp("user.role.normal")));
		test.setRoles(roles_user1);
		test.setState(UserState.ACTIVATED);
		session.save(test);
		AuthUtils.createDir(clusters, test.getUserId());
	...
	public void afterPropertiesSet() throws Exception {
		...
		if(Cloud.getInstance().getProp("build.target").equals("KISTI-NANO")) {
			setting = new ClassPathResource("config.kisti-nano.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-NANO-EN")) {
			setting = new ClassPathResource("config.kisti-nano-en.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-CHEM")) {
			setting = new ClassPathResource("config.kisti-chem.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-CHEM-EN")) {
			setting = new ClassPathResource("config.kisti-chem-en.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("GPLUS")) {
			setting = new ClassPathResource("config.gplus.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("TEST")) {
			setting = new ClassPathResource("config.test.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-TEST")) {
			setting = new ClassPathResource("config.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-CFD")) {
			setting = new ClassPathResource("config.kisti-cfd.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-NAP")) {
			setting = new ClassPathResource("config.kisti-nap.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-CSD")) {
			setting = new ClassPathResource("config.kisti-csd.ini");
		} else if(Cloud.getInstance().getProp("build.target").equals("KISTI-POD")) {
			setting = new ClassPathResource("config.kisti-pod.ini");
		}
		else {
			setting = new ClassPathResource("config.test.ini");
		}
		...
		// for KISTI
		if(Cloud.getInstance().getProp("build.target").equals("KISTI-NANO")) {
			vn =  new VirtualNetwork("1");
		}
		else if(Cloud.getInstance().getProp("build.target").equals("KISTI-CHEM")) {
			vn =  new VirtualNetwork("4");
		}
		else if(Cloud.getInstance().getProp("build.target").equals("KISTI-CHEM-EN")) {
			vn =  new VirtualNetwork("1");
		}
		else if(Cloud.getInstance().getProp("build.target").equals("KISTI-NANO-EN")) {
			vn =  new VirtualNetwork("0");
		}
		else if(Cloud.getInstance().getProp("build.target").equals("GPLUS")) {
			vn = new VirtualNetwork("1");
		} else if(Cloud.getInstance().getProp("build.target").equals("TEST")) {
